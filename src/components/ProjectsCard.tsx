...
const ProjectCards: React.FC<projectcardprops> = ({ value, num }) => {
  ...
  return (
    <FramerWrapper className='max-w-[300px] min-h-[345px] mx-auto' y={0} scale={0.8} delay={num / 4} duration={0.15}>
      <Card className='w-full h-full'>
        ...
        <CardContent>
          ...
          <p className='text-sm font-poppins'>{value.description}</p>
        ...
        <CardFooter className='flex justify-center'>
          <Link href={value.link} target='blank' className={cn(buttonVariants({ variant: 'default', size: 'sm' }), 'flex')}>
